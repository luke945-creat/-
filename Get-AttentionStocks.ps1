...
foreach ($stock in $consecutiveStocks) {
    ...
    if ($p_t4) {
        ...
        # 2. 公告值門檻
        if ($rule6) {
            $up_price_act = Round-ToTaiwanStockTick ([Math]::Round($p_t4 * (1 + $rule6_pct/100), 2)) $true
            $down_price_act = Round-ToTaiwanStockTick ([Math]::Round($p_t4 * (1 - $rule6_pct/100), 2)) $false
    ...
    if ($p_t28) {
        ...
        $up_price = Round-ToTaiwanStockTick ([Math]::Round($p_t28 * (1 + $std_pct/100), 2)) $true
        $up_change = [Math]::Round((($up_price - $currentPrice) / $currentPrice) * 100, 2)
    ...
    if ($p_t58) {
        ...
        $up_price = Round-ToTaiwanStockTick ([Math]::Round($p_t58 * (1 + $std_pct/100), 2)) $true
        $up_change = [Math]::Round((($up_price - $currentPrice) / $currentPrice) * 100, 2)
    ...
    if ($p_t88) {
        ...
        $up_price = Round-ToTaiwanStockTick ([Math]::Round($p_t88 * (1 + $std_pct/100), 2)) $true
        $up_change = [Math]::Round((($up_price - $currentPrice) / $currentPrice) * 100, 2)
